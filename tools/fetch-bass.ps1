...
$packages = @(
    @{ Name = 'bass';      Url = 'https://www.un4seen.com/files/bass24.zip';       Dll = 'bass.dll';      Inner = 'x64/bass.dll';         Note = 'core: WAV, AIFF, MP3, OGG, MP2' }
...
try {
    foreach ($package in $packages) {
        ...
        if ((Test-Path $target) -and -not $Force) {
            Write-Host "  present   $($package.Dll)  - $($package.Note)" -ForegroundColor DarkGray
            $ok++
            continue
        }
        ...
        try {
            Write-Host "  fetching  $($package.Dll)  - $($package.Note)"
            ...
            if (-not $source) {
                throw "x64\$($package.Dll) was not found in the archive"
            }
            ...
            Write-Host "  failed    $($package.Dll): $($_.Exception.Message)" -ForegroundColor Yellow
        }
    }
}
finally {
...
Write-Host ''
Write-Host "Done: $ok of $($packages.Count) libraries in $OutputPath" -ForegroundColor Green
...
if ($failed.Count -gt 0) {
    ...
    Write-Host ''
    Write-Host 'Those formats will not be checked. The libraries can be downloaded by hand' -ForegroundColor Yellow
    Write-Host "from www.un4seen.com; put the 64-bit DLLs into $OutputPath" -ForegroundColor Yellow
}
